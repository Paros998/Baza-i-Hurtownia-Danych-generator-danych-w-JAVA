...
JOIN choroby c ON c.choroby_id = r.choroba_id
...
-- Ranking najlepszej sredniej sprzedazy lekow, w danej placowce, w danym miesice <--------- DO POPRAWY
SELECT l.nazwa AS nazwa_leku, p.nazwa AS nazwa_placowki, p.miasto, AVG (pr.ilosc),
RANK () OVER (PARTITION BY p.nazwa, p.miasto ORDER BY AVG (pr.ilosc) DESC)
...
JOIN h_gabinety g ON g.gabinet_id = w.gabinet_id
JOIN h_placowki p ON p.placowka_id = g.placowka_id
GROUP BY ROLLUP (l.nazwa, p.nazwa, p.miasto)
ORDER BY l.nazwa, ranking;
